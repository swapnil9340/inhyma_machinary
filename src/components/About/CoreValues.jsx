  ...
  return (
        <Box sx={{background :"#E7F4FF" ,display:'grid' ,placeContent:'center', padding : {xs:"30px" ,md:"60px"}, gap:"30px" , marginTop :{xs:"20px" ,md:"20px"}}}>
            <Box sx={{textAlign:'center'}}>
                ...
                <Typography sx={{color :"#243E60", fontSize :{xs:"20px" ,md:"30px"}}}>Buit on Integrity, Sustained by Excellence</Typography>
            </Box>
            <Box 
              sx={{
                '& li::marker': {
            ...
            >
                ...
                {
                    listData.map((item)=>{
                      return  <Typography component={"li"} sx={{fontSize:{xs:"16px" ,md:"25px"} , color : "#606872"}}><span style={{color :"#1955A6"}}>{item.heading}:</span> {item.description}</Typography> 
                    })
                }
            </Box>
        </Box>
